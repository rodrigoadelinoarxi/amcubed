...
class BomImport(models.TransientModel):
    ...
    bom_type = fields.Selection(
        ...
        default='mtp', help='Type of Bill of Materials')
                        ...
                        for col in reader:
                            ...
                            if col[1]:
                                ...
                                product = self.env['product.template'].search(
                                    [(product_search, '=', col[1].strip())], limit=1)
                                ...
                                if col[2]:
                                    ...
                                    variant = self.env[
                                        ...
                                        [(variant_search, '=', col[2].strip())],
                                        limit=1)
                                    bom_data.update({'product_id': variant.id})
                                    bom_data.update({'product_tmpl_id': variant.product_tmpl_id.id})
                                    if col[4]:
                                        uom = self.env['uom.uom'].search(
                                            [('name', '=', col[4].strip())], limit=1)
                                        bom_data.update(
                            ...
                            if col[5]:
                                ...
                                if not variant or variant[variant_search].strip() != str(col[5]).strip():
                                    raise UserError(_('Product Not Found ' + str(col[5])))
            ...
            if self.file_type == 'xls':
                ...
                for rec in self:
                    try:
                        ...
                        for col in ws.iter_rows(min_row=2, values_only=True):
                            ...
                            if col[5]:
                                if rec.product_variant_by == 'default_code':
                                    ...
                                    variant_search = 'barcode'
                                variant = self.env['product.product'].search(
                                    [(variant_search, '=', str(col[5]))], limit=1)
                                if not variant or variant[variant_search].strip() != str(col[5]).strip():
                                    raise UserError(_('Product Not Found ' + str(col[5])))
                                bom_line_dict.update({'product_id': variant.id,
                                                      'bom_id': last_bom_id
                        ...
                        _logger.warning('Line: ' + str(stack_trace))
                        raise UserError(_("Sorry, The Excel file you provided does not match our required format: " + ustr(e)))
